...
destfile <- "R25-Ageing_History_GVBLX.xlsx"
destRES  <- "GVBNL_GI_MATNR.csv"
destBW   <- paste0("./results/", destRES )
pEANTP   <- "Z1"
method   <- "auto"
```

```{r DownloadSourceFile, echo=pEcho, eval=FALSE}
...
setkey(dtMEAN_Z1ST, "MATNR")

# dtMEAN_Z1ST <- dtMEAN_Z1ST[, DMAT := duplicated(dtMEAN_Z1ST, by = c("MATNR"))]

dtGVBNL_GI <- merge(dtGVBNL_GI, dtMEAN_Z1ST, all.x = TRUE, by = "EAN11")
dtGVBNL_GI <- dtGVBNL_GI[!is.na(MATNR)]

dtGVBNL_GI <- dtGVBNL_GI[, DUP  := duplicated(dtGVBNL_GI, 
                                              by = c("MATNR"))]
dtGVBNL_GI <- dtGVBNL_GI[, DUP2 := (sum(DUP) + 1L), by = c("MATNR")]
dtGVBNL_GI <- dtGVBNL_GI[, GIDAT:= max(GIDAT), by = c("MATNR")]
dtGVBNL_GI <- dtGVBNL_GI[DUP == FALSE, ]
dtGVBNL_GI <- dtGVBNL_GI[, `:=`(DUP = NULL, DUP2 = NULL)]

write.table(dtGVBNL_GI, file = "GVBNL_GI_MATNR.csv",sep = ";",
...
```{r}
dtMAPPING <- dtMAPPING[, DUP  := duplicated(dtMAPPING, 
                                              by = c("EAN11"))]
dtMAPPING <- dtMAPPING[, DUP2 := (sum(DUP) + 1L), by = c("EAN11")]
```
