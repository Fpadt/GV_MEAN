...
rbind(
  dtMAPPING[D001 > 0 & EANTP == "Z1", 
            .(SYSTID, MATNR, MEINH, LFNUM, EAN11, EANTP, HPEAN)][1:4],
...
write.table(dtMAPPING[D001 > 0, .(SYSTID, MATNR, MEINH, LFNUM, EAN11, EANTP, HPEAN) ], 
            file = "./RESULTS/RN1.csv", 
...
In total there are `r nrow(tmp)` entries which should not exist! In case Z3 can be ignored only
`r nrow(dtMAPPING[D_ET == TRUE])` remain. 
...
```{r View, echo=fECHO, results=TRUE}
head(dtMAPPING[, .(SYSTID, MATNR, MEINH, LFNUM, EAN11, EANTP, HPEAN) ] , 10)
```
